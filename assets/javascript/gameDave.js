...
//background previously picked is stored in local storage 
var playBackground = localStorage.getItem("battleBackground");
//localStorage.removeItem("battleBackground");
//apply background 
$("body").css("background-image", playBackground);
...
function userCardChoice() {
  ...
  if (playCards.length != 3) {
    m[0] = mainFoods.map(function (e) {
      return e.searchTerm;
    }).indexOf(playerCards[0]);


    m[1] = sideFoods.map(function (e) {
      return e.searchTerm;
    }).indexOf(playerCards[1]);


    m[2] = desserts.map(function (e) {
      return e.searchTerm;
    }).indexOf(playerCards[2]);
...
//     var winData = $("p").text(fireData.wins);
//     var playsData = $("p").text(fireData.plays);

//     $(".firebaseScore").append(winData);
//     $(".firebaseScore").append(playsData);
//   })
